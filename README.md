...
* Don Knuth's [TPK program](http://www-cs-faculty.stanford.edu/~knuth/programs/tpk.i) crashes
at 30218 when given [the suggested inputs](http://www-cs-faculty.stanford.edu/~knuth/programs/tpk.in). 
It fails to retrieve ;4. It looks to me like the program is behaving correctly - it is impossible for ;4 
to have been initialized at that point.  ;4 is initialized on the NEXT line. Other compilers perhaps allow you
to stash an undefined array? I fixed it by adding an initializer near the beginning of the program:
...
I don't have roman numerals as output so it's not trival to check all the outputs but all the 
ones I spot-checked were correct.
...
Knuth's site says it should output
```
X  (10)
